...
public class CursadaTest extends TestCase {
	...
	private Prueba vocabularioBasicoAprobada, vocabularioBasicoDesaprobada, articuloDeterminadoAprobado, presentePerfectoAprobado;
	...
	public void testAgregarPrueba(){
		...
		try {
			encurso.agregarPrueba(presentePerfectoAprobado);
			fail("Agregar una prueba cuya leccion no corresponde al curso de la cursada deberia fallar.");
		} catch (Exception e) {
			assertEquals("No se puede agregar una prueba a una cursada cuando dicha prueba no esta definida para una de las lecciones de dicha cursada.", e.getMessage());
			assertEquals(encurso.getPruebas().size(), 3);
		}
	}
	
	public void testFinalizada(){
